...
public class IssueService {
  ...
  public IssueService(
    ...
    IssueMongoService issueMongoService,
    UserService userService,
    ProjectSprintService projectSprintService,
    IssueMapper taskMapper,
    IssueRepository taskRepository,
    SprintService sprintService,
    UserMapper userMapper,
    PersonalSkillRepository personalSkillRepository
  ) {
    this.issueRelationRepository = issueRelationRepository; this.snapshotService = snapshotService;
    this.taskMapper              = taskMapper; this.taskRepository = taskRepository;
    this.projectSprintService    = projectSprintService; this.userService = userService;
    this.issueMongoService       = issueMongoService; this.changeLogMapper = changeLogMapper;
    this.resourceService         = resourceService; this.projectService = projectService;
    this.sprintService           = sprintService; this.topicMapper = topicMapper; this.subTaskMapper = subTaskMapper;
    this.userMapper              = userMapper; this.personalSkillRepository = personalSkillRepository;
  }
  ...
  public ApiResponse<IssueResponse> createTask(IssueCreateRequest issueCreateRequest) {
    ...
    var task = taskMapper.toEntity(issueCreateRequest); task.setProject(project);
    if (issueCreateRequest.getAssigneeId() != null && !issueCreateRequest.getAssigneeId()
      .isEmpty()) {
      User assignee = userService.getUserByUniId(issueCreateRequest.getAssigneeId()); task.setAssignee(assignee);
    } if (issueCreateRequest.getReviewerId() != null && !issueCreateRequest.getReviewerId()
      .isEmpty()) {
      User reviewer = userService.getUserByUniId(issueCreateRequest.getReviewerId()); task.setReviewer(reviewer);
    } if (issueCreateRequest.getSprintId() != null && !issueCreateRequest.getSprintId()
      .isEmpty()) {
      Sprint sprint = sprintService.getSprintById(issueCreateRequest.getSprintId()); if (task.getDtStart() == null) {
        ...
        List<Map<String, String>> errors = new ArrayList<>(); errors.add(error);
        ...
        Map<String, String> error = new HashMap<>(); error.put("end", "End date cannot be after sprint end date");
        List<Map<String, String>> errors = new ArrayList<>(); errors.add(error);
        throw AppMethodArgumentNotValidException.builder()
          .error(errors)
          .build();
    ...
    } task = taskRepository.save(task); if (task == null || task.getId() == null) {
      throw AppException.builder()
        .error(Error.DB_SERVER_ERROR)
        .build();
    } var taskMongo = taskMapper.toCollection(task, issueCreateRequest); taskMongo = issueMongoService.save(taskMongo);
  ...
  public ApiResponse<IssueResponse> createTaskBacklog(IssueCreateRequest issueCreateRequest) {
    ...
    if (issueCreateRequest.getAssigneeId() != null && !issueCreateRequest.getAssigneeId()
      ...
      .isEmpty()) {
      Sprint sprint = sprintService.getSprintById(issueCreateRequest.getSprintId()); Instant now = ClockSimulator.now();
  ...
  public ApiResponse<IssueDetailResponse> getIssueDetailById(IssueDetailRequest request) {
    ...
    String id = request.getIssueId(); var entity = getEntityById(id);
    if (request.getSprintId() != null && !request.getSprintId()
      .isEmpty()) {
      Sprint sprint = sprintService.getSprintById(id); if (sprint.getDtEnd()
        .isBefore(ClockSimulator.now())) {
        List<IssueSnapshot> snapshots = snapshotService.getByProjectIdAndSprintId(
          entity.getProject()
            .getId(), sprint.getId()
  ...
  public ApiResponse<IssueResponse> updateTask(IssueUpdateRequest updateRequest) {
    String id = updateRequest.getId(); String fliedChanged = updateRequest.getFieldChanging();
    var task = getEntityById(id); var taskMongo = issueMongoService.getById(id); ChangeLogRequest changeLog;
    switch (fliedChanged) {
      case "name":
        task.setName(updateRequest.getName()); task = saveEntity(task); changeLog =
        changeLogMapper.taskToUpdate(new String[]{"name"}, task, taskMongo); break;
      case "description":
        task.setDescription(updateRequest.getDescription()); task.setNumChangeOfDescription(
        task.getNumChangeOfDescription() + 1); task = saveEntity(task); changeLog =
        changeLogMapper.taskToUpdate(new String[]{"description"}, task, taskMongo); break;
      case "sprint":
        Sprint targetSprint = updateRequest.getSprintId() == null || updateRequest.getSprintId()
          .isEmpty()
                              ? null
                              : sprintService.getSprintById(updateRequest.getSprintId()); if (targetSprint == null) {
        // remove sprint
        task.setSprint(null); task.setStatus(IssueStatus.BACKLOG); task = saveEntity(task); changeLog =
          changeLogMapper.taskToUpdate(new String[]{"sprint"}, task, taskMongo); break;
      } Sprint currentSprint = task.getSprint(); Instant now = ClockSimulator.now();
        ...
        if (currentSprint == null || currentSprint.getDtEnd()
          ...
          if (targetSprint.getDtStart()
            ...
            throw AppException.builder()
              ...
              .build();
          } task.setStatus(IssueStatus.TODO); task.setSprint(targetSprint);
          ...
          } task = saveEntity(task); changeLog = changeLogMapper.taskToUpdate(new String[]{"sprint"}, task, taskMongo);
          break;
        }

        if (currentSprint.getDtStart()
          .isAfter(now)) {
          task.setSprint(targetSprint); task = saveEntity(task); changeLog =
            changeLogMapper.taskToUpdate(new String[]{"sprint"}, task, taskMongo); break;
        }
        ...
        throw AppException.builder()
          ...
          .build();
      case "priority":
        task.setPriority(updateRequest.getPriority()); task.setNumChangeOfPriority(task.getNumChangeOfPriority() + 1);

        task = saveEntity(task); changeLog =
        changeLogMapper.taskToUpdate(new String[]{"priority"}, task, taskMongo); break;
      case "status":
        task.setPriority(updateRequest.getPriority()); task = saveEntity(task); changeLog =
        changeLogMapper.taskToUpdate(new String[]{"status"}, task, taskMongo); break;
      case "topics":
        taskMongo.setTopics(topicMapper.toTopicList(updateRequest.getTopics())); taskMongo =
        issueMongoService.saveDocument(taskMongo); changeLog =
        changeLogMapper.taskToUpdate(new String[]{"topics"}, task, taskMongo); break;
      case "subtasks":
        taskMongo.setSubtasks(subTaskMapper.toSubTaskList(updateRequest.getSubtasks())); taskMongo =
        issueMongoService.saveDocument(taskMongo); changeLog =
        changeLogMapper.taskToUpdate(new String[]{"subtasks"}, task, taskMongo); break;
      case "attachments":
        List<Resource> resources = task.getResources(); List<String> newResource = new ArrayList<>(
        updateRequest.getAttachments()
          .stream()
          .map(AttachmentRequest::getResourceId)
          .toList()); resources = resources.stream()
        .filter(r -> !newResource.contains(r.getId()))
        .toList(); newResource.removeAll(resources.stream()
        .map(BaseEntity::getId)
        .toList()); for (String resourceId : newResource) {
        Resource resource = resourceService.getById(resourceId); resources.add(resource);
      } task.setResources(resources); task = saveEntity(task); changeLog =
        changeLogMapper.taskToUpdate(new String[]{"attachments"}, task, taskMongo); break;
      case "assignee":
        if (updateRequest.getAssignee() == null || updateRequest.getAssignee()
          ...
          User assignee = userService.getUserByUniId(updateRequest.getAssignee()); task.setAssignee(assignee);
        } task = saveEntity(task); changeLog =
        changeLogMapper.taskToUpdate(new String[]{"assignee"}, task, taskMongo); break;
      case "reviewer":
        if (updateRequest.getReviewer() == null || updateRequest.getReviewer()
          ...
          User reviewer = userService.getUserByUniId(updateRequest.getReviewer()); task.setReviewer(reviewer);
        } task = saveEntity(task); changeLog =
        changeLogMapper.taskToUpdate(new String[]{"reviewer"}, task, taskMongo); break;
      case "start":
        task.setDtStart(updateRequest.getStart()); if (task.getSprint() != null && task.getDtStart()
        ...
        changeLogMapper.taskToUpdate(new String[]{"start"}, task, taskMongo); break;
      case "end":
        task.setDtEnd(updateRequest.getEnd()); if (task.getSprint() != null && task.getDtEnd()
        ...
        throw AppMethodArgumentNotValidException.builder()
          .error(errors)
          .build();
      } task = saveEntity(task); changeLog = changeLogMapper.taskToUpdate(new String[]{"end"}, task, taskMongo); break;
      default:
        throw AppException.builder()
  ...
  public ApiResponse<List<IssueResponse>> getIssuesBySprintId(IssueOfSprintRequest request) {
    ...
    String sprintId = request.getSprintId(); String projectId = request.getProjectId();
    if (sprintId == null || sprintId.isEmpty()) {
      // check relationship
      List<Issue> issues = projectService.getProjectById(projectId)
        .getIssues(); issues = issues.stream()
        .filter(issue -> issue.getSprint() == null || (issue.getSprint() != null && issue.getSprint()
          .getDtEnd()
          .isBefore(ClockSimulator.now()) && issue.isOpen() && !issue.getStatus()
          .equals(IssueStatus.DONE)))
        .toList(); issues.forEach(issue -> {
        if (!issue.getStatus()
          .equals(IssueStatus.BACKLOG)) {
          issue.setOpen(true); issue.setStatus(IssueStatus.BACKLOG); taskRepository.save(issue);
        }
      }); return buildResponseFromIssues(issues);
    }

    Sprint sprint = sprintService.getSprintById(sprintId); if (ClockSimulator.now()
      .isAfter(sprint.getDtEnd())) {
  ...
  public ApiResponse<IssueResponse> updateTask(IssueUpdateStatusRequest request) {
    String id = request.getId(); var task = getEntityById(id); var taskMongo = issueMongoService.getById(id);
    ChangeLogRequest changeLog; if (request.getStatus()
      .equals(IssueStatus.DONE)) {
      task.setOpen(false); task.setDtEnd(ClockSimulator.now());
    }
    else if (task.getStatus()
               .equals(IssueStatus.DONE) && !request.getStatus()
      .equals(IssueStatus.DONE)) {
      task.setOpen(true); task.setDtEnd(null);
    } task.setStatus(request.getStatus());

    task.setPosition(request.getPosition()); task = saveEntity(task); changeLog = changeLogMapper.taskToUpdate(
      new String[]{
        "status",
        "position"
      }, task, taskMongo
  ...
  public ApiResponse<Boolean> reopen(String id) {
    Issue issue = getEntityById(id); if (issue.getStatus() != IssueStatus.DONE) {
      throw AppException.builder()
        .error(Error.INVALID_PARAMETER_REQUEST)
    ...
    // changed
    issue.setStatus(IssueStatus.BACKLOG); issue.setOpen(true); issue.setSprint(null); issue = saveEntity(issue);
  ...
  private ApiResponse<List<IssueResponse>> buildResponseFromIssues(List<Issue> issues) {
    ...
    List<IssueResponse> responses = issues.stream()
      .map(issue -> {
        var issueMongo = issueMongoService.getById(issue.getId()); return taskMapper.toIssueResponse(issue, issueMongo);
      })
      .toList()
      ;
  ...
  public ApiResponse<IssueDetailResponse> getCurrentIssueDetailById(String id) {
    var entity = getEntityById(id); var taskMongo = issueMongoService.getById(id);
    var taskResponse = taskMapper.toIssueDetailResponse(entity, taskMongo);
    return ApiResponse.<IssueDetailResponse>builder()
      .code(HttpStatus.OK.value())
      .message("Get task detail successfully")
  ...
  public ApiResponse<IssueRelationResponse> createRelation(IssueAssignRelationRequest request) {
    Issue issue = getEntityById(request.getIssueId()); Issue relatedIssue = getEntityById(request.getIssueRelatedId());
  ...
  public ApiResponse<List<IssueRelationResponse>> getRelations(String id) {
    Issue issue = getEntityById(id); List<IssueRelation> relations = issue.getAffectTo();
  ...
  public ApiResponse<List<IssueResponse>> getIssueWithTypeRelation(
    String id,
    String type
  ) {
    Issue issue = getEntityById(id); return switch (IssueRelationType.fromValue(type)) {
      case IssueRelationType.IS_BLOCKED_BY, IssueRelationType.IS_RELATED_TO, IssueRelationType.IS_DEPENDED_ON_BY,
        ...
        List<Issue> issues = taskRepository.findByProjectIdAndIdNot(
            issue.getProject()
              .getId(), issue.getId()
          )
          .orElseThrow(() -> AppException.builder()
            .error(Error.NOT_FOUND)
            .build()); yield buildResponseFromIssues(issues);

      } default -> getIssuesBySprintId(new IssueOfSprintRequest(
        issue.getSprint()
  ...
  public int getNumberOfIssuesAtStart(
    Project project,
    Sprint sprint
  ) {
    return taskRepository.countByProjectIdAndSprintIdAndDtCreatedBefore(
      project.getId(), sprint.getId(),
      sprint.getDtStart()
    );
  }

  public int getNumberOfIssuesAdded(
    Project project,
    Sprint sprint
  ) {
    return taskRepository.countByProjectIdAndSprintIdAndDtCreatedAfter(
      project.getId(), sprint.getId(),
      sprint.getDtStart()
    );
  }

  public int getNumberOfIssuesRemoved(
    Project project,
    Sprint sprint
  ) {
    return taskRepository.countByProjectIdAndSprintIdAndDtEndBeforeAndStatus(
      project.getId(), sprint.getId(),
      ClockSimulator.now(), IssueStatus.DONE
    );
  }

  public int getNumberOfIssuesByStatus(
    Project project,
    Sprint sprint,
    IssueStatus status
  ) {
  ...
  public int getNumberOfMembersInSprint(
    Project project,
    Sprint sprint
  ) {
  ...
  public List<Issue> getIssuesBySprintId(
    String projectId,
    String sprintId
  ) {
  ...
  public int getNumberOfFixVersions(String id) {
    int result = 1; Issue issue = getEntityById(id); var project = issue.getProject();
  ...
  public int getNumberOfLink(String id) {
    Issue issue = getEntityById(id); int result = 0; if (issue.getAffectTo() != null && !issue.getAffectTo()
      .isEmpty()) {
  ...
  public int getNumberOfComments(String id) {
    var issueMongo = issueMongoService.getById(id); if (issueMongo == null || issueMongo.getComments() == null) {
      return 0;
  ...
  public int getNumChangeFixVersion(String id) {
    int result = 0; Issue issue = getEntityById(id); var project = issue.getProject();
    Instant now = ClockSimulator.now();
  ...
  public double calculateCompatibleOfAssignee(Issue issue) {
    User assignee = issue.getAssignee(); if (assignee == null) {
      return 0;
    } var issueMongo = issueMongoService.getById(issue.getId());

    int point = 0; List<Topic> topics = issueMongo.getTopics(); List<PersonalSkill> skills =
      personalSkillRepository.findByUser(assignee)
        .orElse(new ArrayList<>()); for (PersonalSkill skill : skills) {
      if (topics.stream()
