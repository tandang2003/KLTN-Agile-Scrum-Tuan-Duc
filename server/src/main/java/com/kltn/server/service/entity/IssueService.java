...
public class IssueService {
  ...
  public ApiResponse<List<IssueResponse>> getIssueWithTypeRelation(String id, String type) {
    ...
    return switch (IssueRelationType.fromValue(type)) {
      ...
      default -> getIssuesBySprintId(new IssueOfSprintRequest(issue.getSprint().getId(), issue.getProject().getId()));
    };
  }

  public int getNumberOfIssuesAtStart(Project project, Sprint sprint) {
    return taskRepository.countByProjectIdAndSprintIdAndDtCreatedBefore(project.getId(), sprint.getId(), sprint.getDtStart());
  }

  public int getNumberOfIssuesAdded(Project project, Sprint sprint) {
    return taskRepository.countByProjectIdAndSprintIdAndDtCreatedAfter(project.getId(), sprint.getId(), sprint.getDtStart());
  }
  ...
  public double calculateCompatibleOfAssignee(Issue issue) {
    ...
    for (PersonalSkill skill : skills) {
      if (topics.stream().anyMatch(topic -> topic.getName().equals(skill.getSkill().getName()))) {
