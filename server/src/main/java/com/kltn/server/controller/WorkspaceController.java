...
public class WorkspaceController {
  ...
  @GetMapping("/list")
  public ResponseEntity<ApiResponse<ApiPaging<WorkspaceResponse>>> listWorkspaces(
  ...
  @PostMapping("/student")
  @PreAuthorize("hasAuthority('manage_workspace_members')")
  public ResponseEntity<ApiResponse<Void>> addStudentToWorkspace(
      @Valid @RequestBody WorkspaceUserAdditionRequest workspaceUserAdditionRequest) {
    return ResponseEntity.ok().body(
        workspaceService.addStudentToWorkspace(workspaceUserAdditionRequest.workspaceId(),
            workspaceUserAdditionRequest.studentIds()));
  }

  @GetMapping("/{workspaceId}/project")
  @PreAuthorize("hasRole('TEACHER')")
  public ResponseEntity<ApiResponse<ApiPaging<ProjectResponse>>> getProjectByWorkspaceId(
      @PathVariable String workspaceId,
      @RequestParam(defaultValue = "0") int page,
      @RequestParam(defaultValue = "10") int size) {
    ApiResponse<ApiPaging<ProjectResponse>> projectResponse = workspaceService.getListPagingProject(workspaceId, page,
        size);
    return ResponseEntity.status(projectResponse.getCode()).body(projectResponse);
  }

  @GetMapping("/project/user-info")
  public ResponseEntity<ApiResponse<WorkspaceAuthorizationResponse>> getUserInfoInProject(
      @RequestParam @NotEmpty String workspaceId) {
