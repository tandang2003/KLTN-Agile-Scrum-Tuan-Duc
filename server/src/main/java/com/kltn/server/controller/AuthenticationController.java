...
public class AuthenticationController {
  ...
  @Autowired
  public AuthenticationController(@Qualifier("refreshTokenDecoder")
                                  JwtDecoder jwtDecoder, AuthenticationService authenticationService, UserTokenService userTokenService, CookieUtils cookieUtils) {
  ...
  public ResponseEntity<ApiResponse<AuthenticationResponse>> refresh(
    ...
    if (userTokenService.isRefreshTokenExpired((String) jwt.getClaim("sail") + (String) jwt.getClaim("uniId"))) {
      throw AppException.builder().error(Error.TOKEN_INVALID).build();
    }
    AuthenticationResponse response = authenticationService.refresh(jwt);
    return ResponseEntity.ok()
      .body(
        ApiResponse
          .<AuthenticationResponse>builder()
  ...
  public ResponseEntity<ApiResponse<Boolean>> registerTeacher(
    @Valid @RequestBody TeacherRegisterRequest registerRequest) {
    authenticationService.register(registerRequest);
    return ResponseEntity.ok().body(ApiResponse.<Boolean>builder().build());
  }
